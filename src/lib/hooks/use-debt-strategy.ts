...
export function useDebtStrategy(
  ...
  const metrics = useMemo(() => {
    ...
    const debtToIncomeRatio = monthlyIncome
      ...
      : null;
    const weightedAverageInterestRate = calculateWeightedAverageInterestRate(
      debts,
    );
  ...
  return useMemo(() => {
    ...
    if (comparison.comparison.interestSavingsWithAvalanche > 1000) {
      recommendations.push({
        ...
        description: `Save $${
          comparison.comparison.interestSavingsWithAvalanche.toFixed(
            2,
          )
        } in interest`,
        priority: "high",
    ...
    if (comparison.comparison.motivationalBenefitOfSnowball >= 2) {
      recommendations.push({
        ...
        description:
          `Eliminate ${comparison.comparison.motivationalBenefitOfSnowball} debts in the first year`,
        priority: "medium",
        reasoning:
...
export function useDebtProgress(
  ...
  return useMemo(() => {
    ...
    const progressPercentage = originalTotal > 0
      ? (totalPaid / originalTotal) * 100
      : 0;

    const debtsEliminated = originalDebts.length - currentDebts.length;
    const averageMonthlyProgress = monthlyBudget > 0
      ? totalPaid / monthlyBudget
      : 0;
